...
// For direct access (not from Telegram webhook)
if ($_SERVER['REQUEST_METHOD'] === 'GET' && empty(file_get_contents('php://input'))) {
    ...
    <body>
        <div class='container'>
            <h1>🎬 Entertainment Tadka Telegram Bot</h1>
            <div class='status success'>✅ Bot is running on Render.com</div>
    ...
    $files = [
        ...
        'movie_requests.json' => 'Movie Requests',
        'bot_activity.log' => 'Activity Log'
    ];
    ...
    foreach ($files as $file => $desc) {
        if (file_exists($file)) {
            ...
            echo "<p>📁 <strong>$desc:</strong> <span class='success'>✅ Exists</span> ($size_kb KB)</p>";
        } else {
            echo "<p>📁 <strong>$desc:</strong> <span class='warning'>⚠️ Missing</span></p>";
        }
    }
    ...
    if (file_exists('backups') && is_dir('backups')) {
        ...
        echo "<p>💾 <strong>Backups:</strong> <span class='success'>✅ Directory exists</span> ($backup_count backups)</p>";
    } else {
        echo "<p>💾 <strong>Backups:</strong> <span class='warning'>⚠️ Directory missing</span></p>";
    }
    ...
    echo "</div>
        ...
        <div class='section'>
            ...
            <a href='?checkcsv=1' class='btn'>Check CSV</a>
            <a href='?stats=1' class='btn'>View Stats</a>
        </div>
        
        <div class='section'>
            <h3>🔧 Setup Instructions</h3>
            <ol>
                <li>Set webhook using button above</li>
                <li>Test with <code>/start</code> command in Telegram</li>
                <li>Add movies via channel posts</li>
        ...
        <div class='section'>
            <h3>📞 Support</h3>
            <p>For issues, contact: @EntertainmentTadka0786</p>
            <p>Join channels:</p>
            <ul>
                <li>🍿 Main: @EntertainmentTadka786</li>
                <li>📥 Requests: @EntertainmentTadka7860</li>
                <li>🎭 Theater: @threater_print_movies</li>
                <li>🔒 Backup: @ETBackup</li>
            </ul>
        </div>
        
        <div class='status info'>
            <strong>Note:</strong> This page only shows when accessed directly. Telegram webhook requests are processed automatically.
        </div>
...
if (!MAIN_CHANNEL_ID || !THEATER_CHANNEL_ID || !BACKUP_CHANNEL_ID) {
    error_log("❌ Essential channel IDs environment variables set nahi hain. Render.com dashboard mein set karo.");
    echo json_encode(['ok' => false, 'error' => 'Channel IDs not configured']);
    exit;
}

// File paths - Yeh sab files bot ke saath create hongi
// UPDATED: CSV FORMAT LOCKED - Only movie_name, message_id, channel_id
...
function initialize_files() {
    ...
    // UPDATED: CSV FORMAT LOCKED - Only 3 columns
    $files = [
        CSV_FILE => "movie_name,message_id,channel_id\n",  // UPDATED: Only 3 columns locked
    ...
    if (!file_exists(BACKUP_DIR)) {
        ...
        error_log("Created backup directory: " . BACKUP_DIR);
    }
...
function bot_log($message, $type = 'INFO') {
    ...
    // Also log to error log for Render.com monitoring
    if ($type === 'ERROR' || $type === 'WARNING') {
        error_log("Bot $type: $message");
    }
}
...
// ==============================
// CSV MANAGEMENT FUNCTIONS - UPDATED FOR LOCKED FORMAT
// ==============================
function load_and_clean_csv($filename = CSV_FILE) {
    global $movie_messages;
    
    // CSV file check karo, agar nahi hai toh create karo
    // UPDATED: Only 3 columns in header
    ...
    if ($handle !== FALSE) {
        ...
        while (($row = fgetcsv($handle)) !== FALSE) {
            if (count($row) >= 3 && (!empty(trim($row[0])))) {
                ...
                // Channel type determine karo channel ID se
    ...
    // CSV clean karo aur rewrite karo - UPDATED: Only 3 columns
...
function show_typing_indicator($chat_id) {
...
function show_uploading_indicator($chat_id) {
    // "Uploading photo/video..." action show karta hai
    apiRequest('sendChatAction', [
        'chat_id' => $chat_id,
        'action' => 'upload_photo'
    ]);
...
function show_recording_indicator($chat_id) {
    // "Recording video/audio..." action show karta hai
    apiRequest('sendChatAction', [
        'chat_id' => $chat_id,
        'action' => 'record_video'
    ]);
...
function smart_typing_handler($chat_id, $action_type = 'typing', $duration = 3) {
    // Smart typing indicator with auto-cancel
    static $typing_timers = [];
    
    $timer_key = $chat_id . '_' . $action_type;
    
    // Agar pehle se chal raha hai toh stop karo
    if (isset($typing_timers[$timer_key])) {
        return;
    }
    ...
    $action = $action_map[$action_type] ?? 'typing';
    ...
    // Timer set karo (max 5 seconds Telegram limit)
    $typing_timers[$timer_key] = time();
    
    // Auto cleanup after duration
    if ($duration > 0) {
        register_shutdown_function(function() use ($timer_key, $duration) {
            sleep(min($duration, 5));
            unset($GLOBALS['typing_timers'][$timer_key]);
        });
    }
}

// ==============================
// MOVIE DELIVERY SYSTEM - ALL CHANNELS SUPPORT WITH FORWARDING CONTROL
// ==============================
function deliver_item_to_chat($chat_id, $item) {
    // Movie user ko deliver karta hai - WITH SMART FORWARDING CONTROL
    ...
    // SMART FORWARDING CONTROL: Public channels ke liye forward, private ke liye copy
    $is_public = is_public_channel($channel_type);
    
    // Agar valid message ID hai toh
    if (!empty($item['message_id']) && is_numeric($item['message_id'])) {
        if ($is_public) {
            // PUBLIC CHANNEL: Forward use karo (forwarded from dikhega)
            ...
            // PRIVATE CHANNEL: Copy use karo (forwarded from nahi dikhega)
...
// ==============================
// LANGUAGE DETECTION - UPDATED WITH HINGLISH
// ==============================
function detect_language($text) {
    // Text ki language detect karta hai (Hindi/English/Hinglish)
    $hindi_keywords = ['फिल्म', 'मूवी', 'डाउनलोड', 'हिंदी', 'चाहिए', 'कहाँ', 'कैसे', 'खोज', 'तलाश', 'प्लीज', 'भाई', 'बॉस', 'जी'];
    $english_keywords = ['movie', 'download', 'watch', 'print', 'search', 'find', 'looking', 'want', 'need', 'please', 'bro', 'boss', 'sir'];
    ...
    // Hinglish detection - Mixed pattern
    if ($hindi_score > 0 && $english_score > 0) {
        // Mixed Hindi and English words
        return 'hinglish';
    } elseif ($hindi_score > $english_score) {
        return 'hindi';
    } else {
        return 'english';
    }
...
// ==============================
// MULTILINGUAL RESPONSES - UPDATED WITH HINGLISH
// ==============================
function send_multilingual_response($chat_id, $message_type, $language) {
    ...
    $responses = [
        ...
        'hinglish' => [
            'welcome' => "🎬 Boss, kaunsi movie search kar rahe ho?",
            'found' => "✅ Mil gayi! Movie ka info bhej raha hun...",
            'not_found' => "😔 Yeh movie abhi available nahi hai bro!\n\n📝 Aap isko request kar sakte ho: " . REQUEST_CHANNEL . "\n\n🔔 Jab bhi yeh add hogi, main automatically bhej dunga!",
            'searching' => "🔍 Searching kar raha hun... Thoda wait karo",
            'multiple_found' => "🎯 Multiple versions mili hain! Aap kaunsi chahiye?",
            'request_success' => "✅ Request receive ho gayi! Hum jald hi add karenge.",
            'request_limit' => "❌ Aaj ke liye aap maximum " . DAILY_REQUEST_LIMIT . " requests hi kar sakte ho bhai."
        ]
    ];
...
// ==============================
// SMART SEARCH WITH "DID YOU MEAN" SUGGESTION
// ==============================
function get_did_you_mean_suggestions($query, $all_movie_names) {
    // Similar movie names suggest karta hai
    $query_lower = strtolower(trim($query));
    $suggestions = [];
    
    foreach ($all_movie_names as $movie_name) {
        $movie_lower = strtolower($movie_name);
        
        // Exact match check
        if ($movie_lower == $query_lower) {
            return []; // Exact match mil gaya, suggestions nahi chahiye
        }
        
        // Similarity calculate karo
        similar_text($query_lower, $movie_lower, $similarity);
        
        // Agar similarity 70% se zyada hai toh suggestion mein add karo
        if ($similarity > 70) {
            $suggestions[] = [
                'name' => $movie_name,
        ...
        // Partial match check
        if (strpos($movie_lower, $query_lower) !== false || strpos($query_lower, $movie_lower) !== false) {
            // Already added nahi hai toh add karo
            $already_exists = false;
            foreach ($suggestions as $suggestion) {
                if ($suggestion['name'] == $movie_name) {
                    $already_exists = true;
                    break;
                }
            }
            ...
            if (!$already_exists) {
                $suggestions[] = [
                    'name' => $movie_name,
                    'similarity' => 80 // Partial match ko higher priority
                ];
            }
        }
    }
    
    // Similarity ke hisab se sort karo
    usort($suggestions, function($a, $b) {
        return $b['similarity'] - $a['similarity'];
    });
    
    // Top 3 suggestions return karo
...
function advanced_search($chat_id, $query, $user_id = null) {
    ...
    if (!empty($found)) {
        ...
        // "Did you mean" suggestions add karo
        $all_movie_names = array_keys($movie_messages);
        $suggestions = get_did_you_mean_suggestions($q, $all_movie_names);
        
        $lang = detect_language($query);
        send_multilingual_response($chat_id, 'not_found', $lang);
        
        // Agar suggestions hain toh show karo
        if (!empty($suggestions)) {
            $suggestions_msg = "🤔 <b>Did you mean:</b>\n\n";
            foreach ($suggestions as $index => $suggestion) {
                $suggestions_msg .= ($index + 1) . ". " . $suggestion['name'] . "\n";
            }
            
            $suggestions_msg .= "\n💡 <i>Click on suggestion to search</i>";
            
            $suggestions_keyboard = ['inline_keyboard' => []];
            foreach ($suggestions as $suggestion) {
                $suggestions_keyboard['inline_keyboard'][] = [[
                    'text' => "🔍 " . $suggestion['name'],
                    'callback_data' => 'search_' . base64_encode($suggestion['name'])
                ]];
            }
            
            sendMessage($chat_id, $suggestions_msg, $suggestions_keyboard, 'HTML');
        }
    ...
    return [
        'inline_keyboard' => [
            [
                [
                    'text' => '⏮ First',
                    'callback_data' => 'tu_first_1'
                ],
                [
                    'text' => "🔘 Mid ($mid)",
                    'callback_data' => 'tu_mid_' . $mid
                ],
                [
                    'text' => '⏭ Last',
                    'callback_data' => 'tu_last_' . $total_pages
                ]
            ],
            [
                [
                    'text' => "📄 Page $current_page / $total_pages",
                    'callback_data' => 'tu_current'
                ]
            ],
            [
                [
                    'text' => '📤 Send This Page',
                    'callback_data' => 'tu_view_' . $current_page
                ],
                [
                    'text' => '❌ Stop',
...
// ==============================
// TOTALUPLOAD CONTROLLER - SIMPLE VERSION
// ==============================
function totalupload_controller_simple($chat_id, $page = 1) {
    // Typing indicator show karo
    show_typing_indicator($chat_id);
    
    $movies = get_all_movies_list(); // file_id array

    if (empty($movies)) {
        sendMessage($chat_id, "📭 <b>No uploads found!</b>", null, 'HTML');
        return;
    }
    ...
    $slice = array_slice($movies, $offset, $per_page);

    // Forward current page uploads with smart forwarding control
    ...
    $text .= "📊 <b>Total Movies:</b> $total\n";
    $text .= "📄 <b>Page:</b> $page / $total_pages\n";
    $text .= "🎞 <b>Showing:</b> " . count($slice) . " movies\n\n";
    $text .= "📢 Join: @EntertainmentTadka786";

    $kb = build_totalupload_keyboard($page, $total_pages);
    sendMessage($chat_id, $text, $kb, 'HTML');
}

function build_totalupload_keyboard_enhanced(int $page, int $total_pages, string $session_id = '', array $filters = []): array {
    $kb = ['inline_keyboard' => []];
...
function totalupload_controller($chat_id, $page = 1, $filters = [], $session_id = null) {
    ...
    $kb = build_totalupload_keyboard_enhanced($pg['page'], $pg['total_pages'], $session_id, $filters);
...
function show_csv_data($chat_id, $show_all = false) {
    ...
    foreach ($movies as $movie) {
        ...
        $date = date('d-m-Y'); // Current date since format locked
        $quality = 'Unknown'; // Default since format locked
        $language = 'Hindi'; // Default since format locked
        $channel_type = get_channel_type_by_id($movie[2] ?? '');
...
function check_date($chat_id) {
    ...
    if ($h !== FALSE) {
        ...
        while (($r = fgetcsv($h)) !== FALSE) {
            if (count($r) >= 3) {
                $d = date('d-m-Y'); // Current date since format locked
...
function test_csv($chat_id) {
    ...
    if ($h !== FALSE) {
        ...
        while (($r = fgetcsv($h)) !== FALSE) {
            if (count($r) >= 3) {
                $channel_type = get_channel_type_by_id($r[2] ?? '');
                $channel_icon = get_channel_display_name($channel_type);
                $line = "$i. $channel_icon {$r[0]} | ID/Ref: {$r[1]} | Channel: {$r[2]}";
                $line .= "\n";
...
function show_bot_info($chat_id) {
    ...
    $message .= "• Multi-language support (Hindi/English/Hinglish)\n";
...
// ==============================
// MOVIE APPEND FUNCTION WITH AUTO-NOTIFICATION - UPDATED FOR LOCKED FORMAT
// ==============================
function append_movie($movie_name, $message_id_raw, $date = null, $video_path = '', $quality = 'Unknown', $size = 'Unknown', $language = 'Hindi', $channel_type = 'main') {
    // Movie database mein add karta hai - UPDATED FOR LOCKED FORMAT
    ...
    bot_log("Movie appended: $movie_name with ID $message_id_raw to $channel_type channel ($channel_id) - LOCKED FORMAT");
}
...
function handle_command($chat_id, $user_id, $command, $params = []) {
    ...
    switch ($command) {
        ...
        case '/browse':
            ...
            // Use simple totalupload controller
            totalupload_controller_simple($chat_id, $page);
            break;
...
if (isset($_GET['setwebhook'])) {
    ...
    <body>
        <h1>🎬 Webhook Setup - Render.com</h1>
        <div class='info'>
    ...
    $webhook_info = json_decode(apiRequest('getWebhookInfo'), true);
    echo "<h2>🔧 Webhook Info</h2><pre>" . print_r($webhook_info, true) . "</pre>";
...
if (isset($_GET['test'])) {
    ...
    $bot_info = json_decode(apiRequest('getMe'), true);
    if ($bot_info && isset($bot_info['ok']) && $bot_info['ok']) {
        echo "<p class='success'>✅ Bot API connection successful</p>";
        echo "<p>Bot: @" . htmlspecialchars($bot_info['result']['username']) . "</p>";
    } else {
        echo "<p class='error'>❌ Bot API connection failed</p>";
    }
    ...
    foreach ($files as $file => $desc) {
        if (file_exists($file)) {
            $size = filesize($file);
            echo "<p>✅ $desc: " . htmlspecialchars($file) . " (" . $size . " bytes)</p>";
        } else {
    ...
    if (file_exists(CSV_FILE)) {
        $csv_data = load_and_clean_csv();
        echo "<p>✅ CSV loaded: " . count($csv_data) . " movies</p>";
    }
...
if (isset($_GET['checkcsv'])) {
    ...
    if ($handle !== FALSE) {
        ...
        <tr><th>#</th><th>Movie Name</th><th>Message ID</th><th>Channel ID</th></tr>";
        
        $i = 1;
        fgetcsv($handle); // skip header
        while (($row = fgetcsv($handle)) !== FALSE) {
            if (count($row) >= 3) {
                $channel_type = get_channel_type_by_id($row[2]);
                $channel_name = get_channel_display_name($channel_type);
                echo "<tr>
                    <td>$i</td>
                    <td>" . htmlspecialchars($row[0]) . "</td>
                    <td>" . htmlspecialchars($row[1]) . "</td>
                    <td title='$channel_name'>" . htmlspecialchars($row[2]) . "</td>
                </tr>";
                $i++;
            }
        }
...
if (isset($_GET['stats'])) {
    // Display statistics
    echo "<h1>📈 Bot Statistics</h1>";
    
    $stats = get_stats();
    echo "<pre>" . print_r($stats, true) . "</pre>";
    
    $users_data = json_decode(file_get_contents(USERS_FILE), true);
    echo "<h2>👥 Users (" . count($users_data['users'] ?? []) . ")</h2>";
    echo "<pre>" . print_r($users_data, true) . "</pre>";
    
    echo "<p><a href='?'>← Back to Status Page</a></p>";
    exit;
}

// Main Telegram webhook processing
...
if ($update) {
    ...
    if (isset($update['channel_post'])) {
        ...
        if (!empty(trim($text))) {
            // UPDATED: Only 3 parameters as per locked format
    ...
    if (isset($update['callback_query'])) {
        ...
        // TOTALUPLOAD CONTROLS - SIMPLE VERSION
        elseif ($data === 'tu_first_1') {
            totalupload_controller_simple($chat_id, 1);
            answerCallbackQuery($query['id'], "First page");
        }
        elseif (strpos($data, 'tu_mid_') === 0) {
            $page = (int)str_replace('tu_mid_', '', $data);
            totalupload_controller_simple($chat_id, $page);
            answerCallbackQuery($query['id'], "Page $page");
        }
        elseif (strpos($data, 'tu_last_') === 0) {
            $page = (int)str_replace('tu_last_', '', $data);
            totalupload_controller_simple($chat_id, $page);
        ...
        elseif (strpos($data, 'tu_view_') === 0) {
            ...
            $per_page = 10;
            $total_pages = (int)ceil(count($movies) / $per_page);
            $page = max(1, min($page, $total_pages));
            $offset = ($page - 1) * $per_page;
            $slice = array_slice($movies, $offset, $per_page);
        ...
        elseif (strpos($data, 'send_') === 0) {
            ...
            foreach ($pg['slice'] as $item) {
                deliver_item_to_chat($chat_id, $item);
                sleep(1);
            }
            answerCallbackQuery($query['id'], "📦 Batch info sent!");
        }
        // Preview page
        ...
        elseif (strpos($data, 'search_') === 0) {
            $movie_name = base64_decode(str_replace('search_', '', $data));
            advanced_search($chat_id, $movie_name, $user_id);
            answerCallbackQuery($query['id'], "Searching: $movie_name");
        }
...
if (isset($_GET['test_save'])) {
    ...
    function manual_save_to_csv($movie_name, $message_id, $channel_type = 'main') {
    ...
    manual_save_to_csv("Metro In Dino (2025)", 1924, "main");
    manual_save_to_csv("Metro In Dino 2025 WebRip 480p", 1925, "main");
    manual_save_to_csv("Metro In Dino (2025) Hindi 720p", 1926, "main");
    manual_save_to_csv("Animal (2023) Hindi 1080p", 1927, "main");
    manual_save_to_csv("Avengers Endgame (2019) English", 1928, "main");
...
// ==============================
// DEFAULT RESPONSE FOR WEBHOOK REQUESTS
// ==============================

// Agar koi bhi update nahi hai, toh empty response send karo
// (Telegram expects 200 OK for webhook requests)
if (!$update && !isset($_GET['setwebhook']) && !isset($_GET['test']) && 
    !isset($_GET['checkcsv']) && !isset($_GET['stats']) && !isset($_GET['test_save'])) {
    
    // Webhook verification ke liye empty JSON response
    echo json_encode(['ok' => true, 'description' => 'Telegram bot webhook endpoint']);
}